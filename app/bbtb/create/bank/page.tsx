...
import { CreateBankFormInputs } from "@/constant/typeConst";
...
export default function bank (){
    ...
    const {
        ...
        handleSubmit,
        formState: { errors },
    } = useForm<CreateBankFormInputs>({
        resolver: zodResolver(schema),
    });
    //フォームボタン処理
    const onSubmit: SubmitHandler<CreateBankFormInputs> = async (data) => {
        console.log(data)
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    ...
                    >
                        <Box display="flex" justifyContent="center" alignItems="center">
                            <Text fontSize="2xl">銀行情報を登録</Text>
                        </Box>
                        ...
                        <Box marginTop="3%">
                            <Text>銀行名を選択</Text>
                            <Input type="text" {...register('bank_name')}></Input>
                            {errors.bank_name && <Text color="red">{errors.bank_name.message?.toString()}</Text>}
                        </Box>
                        <Box marginTop="3%">
                            <Text>現在の口座残高</Text>
                            <Input type="number" {...register('balance', { valueAsNumber: true })}></Input>
